...
from corruption.Corruption_transform import uncorrelated_corruption_transform, correlated_corruption_transform
...
def extract_query_or_gall_feat(query_gall_loader, n_query_gall, net, modality="VtoV", QorG="Query", model="concatenation"):
    ...
    pool_size={"transreid":3840, "LMBN":3584, "unimodal":512, "concatenation":1024, "MMSF":1536}
    tp_model = "MMSF" if "MMSF_" in model else model
    query_gall_feat_pool = np.zeros((n_query_gall, pool_size[tp_model]))
    query_gall_feat_fc = np.zeros((n_query_gall, pool_size[tp_model]))
    ...
    with torch.no_grad():
        for batch_idx, (input1, input2, *label) in enumerate(query_gall_loader):
            ...
            if model == "transreid":
                ...
                feat_pool, feat_fc, *_ = net([input1, None, input2])
            else :
...
def transform_for_test(img_height, img_width, scenario_eval, model, CCD_X):
    transform_list = []
    transform_list.append(transforms.ToPILImage())

    ## C* to change with UCD
    if scenario_eval in ["C", "UCD"]: # If star we corrupt both RGB and IR else we corrupt
        transform_list.append(uncorrelated_corruption_transform(0, type='all'))
    elif scenario_eval in ["CCD"]:
        transform_list.append(correlated_corruption_transform(0, CCD_X=CCD_X))
    if model == "transreid":
    ...
    return Compose(transform_list, CIL="Not_used_for_test", XPATCH="Not_used_for_test", scenario_eval=scenario_eval)
...
def MMSF_mid_stream(model_name):
    mid_strm_fus_blck = -1
    for i in range(5):
        if str(i) in model_name:
            mid_strm_fus_blck = i
    if mid_strm_fus_blck == -1:
        exit(f" Issue with the ls value should not be -1 (train.py)")
    return mid_strm_fus_blck
