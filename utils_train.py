...
def train(epoch, lr, net, reid, trainloader, device, optimizer, criterion_id, criterion_tri,
    ...
    for batch_idx, (input1, input2, label1, label2) in enumerate(trainloader):
        ...
        elif "MMSF_" in model_name:
            feat, out0, feat1, out1, feat2, out2 = net([input1, None, input2])
        else:
        ...
        if (Augmix or XPATCH!="False" or XREA!="False" or ML_MDA):
            ...
            if "MMSF_" in model_name: # The model has two other streams to individually process
                loss = loss_ce + loss_tri
                loss_ce_contour, loss_tri_contour = criterion_id(out1, feat1, labels)
                loss += loss_ce_contour + loss_tri_contour
                loss_ce_IR, loss_tri_IR = criterion_id(out2, feat2, labels)
                loss += loss_ce_IR + loss_tri_IR
            ...
            # Global feat count for 0.5 and locals for 0.55
            ...
            loss_ce = criterion_id(out0, labels) + criterion_id(out1, labels) + criterion_id(out2, labels)
            loss_tri_and_batch_acc = [criterion_tri(feat,labels), criterion_tri(feat1,labels), criterion_tri(feat2,labels)]
            loss_tri, batch_acc = tuple(sum(x) for x in zip(*loss_tri_and_batch_acc))
        ...
        if "MMSF_" in model_name:
            predicted = (predicted + out1.max(1)[1] + out1.max(1)[1])/3 # Prediction accuracy is avg over the three streams
