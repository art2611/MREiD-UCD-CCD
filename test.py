...
from model import Global_network, MMSF
import argparse
from utils import prepare_save_folder_or_get_path_models, create_folder, extract_query_or_gall_feat,\
    transform_for_test, MMSF_mid_stream
...
parser.add_argument('--model', default='concatenation', help='unimodal, concatenation, transreid, LMBN, MMSF_X')
...
parser.add_argument('--scenario_eval', default='normal', help='Evaluation type: normal, C, UCD, CCD')
parser.add_argument('--CCD_X', default=0, type=int, help='For CCD, percentage X of pairs for which uncorrelated'
                                                         'corruption is paired with a clean image. Default is 0'
                                                         'meaning each pair has the two modalities corrupted')
...
if args.scenario_eval!="CCD":
    results_folder = f"{args.results}/{args.scenario_eval}/{args.dataset}/"
else :
    results_folder = f"{args.results}/{args.scenario_eval}-{args.CCD_X}/{args.dataset}/"
...
transform_test = transform_for_test(img_h, img_w, args.scenario_eval, args.model, args.CCD_X)
...
if args.dataset == "SYSU": # SYSU anytime
    trials = 30
elif args.scenario_eval in ["C", "UCD", "CCD"] : # Evaluation over 10 trials for RegDB or TWorld if Corrupted data
    trials = 10
...
for fold in range(folds):
    ...
    elif "MMSF_" in args.model:
        middle_stream_fusion_block = MMSF_mid_stream(args.model)
        net.append(MMSF(nclass[args.dataset], middle_stream_fusion_block))
    else :
...
message = data_info.replace("fold_4_", "")
...
g_message = now + " " + data_info.replace("fold_4_", "") + \
