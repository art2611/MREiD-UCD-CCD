...
from utils import IdentitySampler, prepare_save_folder_or_get_path_models, transform_for_train, \
    saveModel_displayResults, save_curves, save_first_batch_imgs, MMSF_mid_stream
...
model_list.extend([f"MMSF_{i}" for i in ["0", "1", "2", "3", "4"]])

assert args.model in model_list, f'--model={args.model} but should be in {model_list}'
...
if args.Augmix or args.CIL or args.ML_MDA:  # All of those automatically use augmix per default
    suffix += "_augmix"
if args.XREA in ["S-REA", "MS-REA"]:
    suffix += f"_{args.XREA}"
if args.ML_MDA :
    suffix += f"_MS-REA"
if args.XPATCH in ['S-PATCH', 'MS-PATCH', 'M-PATCH-SS', 'M-PATCH-SD', 'M-PATCH-DD']:
...
if args.model == "transreid":
    ...
    net = MMSF(n_class, middle_stream_fusion_block)
elif args.model in ["unimodal", "concatenation"]:  # Unimodal / Concatenation
